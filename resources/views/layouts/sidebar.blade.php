<aside id="sidebar"
    ...
    <a href="/" wire:navigate class="mb-3 px-2 pb-2 flex rounded-md">
        <img src="{{ asset('assets/img/Resolute1.jpg') }}" alt="Logo" class="w-32">
    </a>

    <a href="/"
       class="logo-wrapper p-2 flex items-center gap-2 bg-white/90 text-slate-700 border rounded-md border-white/70 shadow transition hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40">
        <div
            class="flex items-center justify-center h-10 w-10 bg-[#ffd324] rounded text-white border border-[#edcb34] shadow-lg shrink-0">
            <svg viewBox="0 0 24 24" fill="currentColor" class="w-6" aria-hidden="true">
                ...
                <path fill="currentColor"
                    d="M14 9h4v2h-4zm1 4h3v2zm-1.57 2.536c0-1.374-1.676-2.786-3.715-2.786S6 14.162 6 15.536V16h7.43z">
                </path>
            </svg>
        </div>
        <div class="flex flex-col">
            <span class="font-semibold text-sm">Gate Pass Management</span>
            <span class="text-xs text-slate-500">{{ now()->format('l, F j, Y') }}</span>
        </div>
    ...
    <div class="flex flex-col h-full overflow-y-auto pr-1">
        ...
        <h2 class="font-medium ml-4 uppercase tracking-wider text-white bold mb-4">Menu</h2>

        <ul class="flex flex-col gap-2">
            <li>
                <a wire:navigate href="/"
                   class="active group flex items-center gap-3 py-2 px-4 rounded-md bg-white/10 text-white shadow-inner transition-colors hover:bg-white/15 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40">
                    <svg class="w-5 h-5 text-white/90 group-hover:text-white shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                         viewBox="0 0 24 24">
            ...
            <li>
                <details class="group">
                    <summary
                        class="flex items-center gap-3 px-4 py-2 rounded transition-colors hover:bg-white/10 hover:text-white cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40">
                        <svg class="w-5 h-5 text-white/90 group-hover:text-white shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                             viewBox="0 0 24 24">
                        ...
                        <svg class="ml-auto w-5 h-5 text-white/80 transition-transform duration-200 group-open:rotate-180" fill="none" stroke="currentColor" stroke-width="2"
                             viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"></path>
                        </svg>
                    </summary>
                    <ul class="submenu mt-1 ml-4 border-l border-white/10 pl-2 space-y-1">
                        <li>
                            <a wire:navigate href="{{ route('user.index') }}"
                               class="flex items-center gap-3 px-4 py-2 rounded transition-colors hover:bg-white/10 hover:text-white text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40">
                                <svg class="w-5 h-5 text-white/80 shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                                     viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <ul class="submenu mt-1 ml-4 border-l border-white/10 pl-2 space-y-1">
                        <li>
                            <a wire:navigate href="{{ route('car.create') }}"
                               class="flex items-center gap-3 px-4 py-2 rounded transition-colors hover:bg-white/10 hover:text-white text-sm">
                                <svg class="w-5 h-5 text-white/80 shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                                     viewBox="0 0 24 24">
                    ...
                    <ul class="submenu mt-1 ml-4 border-l border-white/10 pl-2 space-y-1">
                        <li>
                            ...
                            <a wire:navigate href="{{ route('car.pending') }}"
                               class="mt-1 flex items-center gap-3 px-4 py-2 rounded transition-colors hover:bg-white/10 hover:text-white text-sm">
                                <svg class="w-5 h-5 text-white/80 shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                                     viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3" />
                                    <circle cx="12" cy="12" r="10" />
        ...
        <div class="mt-auto border border-white/15 rounded-xl px-4 py-3 bg-white/5">
            <details class="group">
                <summary class="flex items-center gap-3 list-none cursor-pointer rounded hover:bg-white/10 px-2 py-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40">
                    <div class="flex items-center justify-center w-10 h-10 bg-white/80 rounded-full border border-white/60 text-[#0E3A61] font-semibold uppercase">
                        O
                    </div>
                    <div class="flex flex-col max-w-[160px] truncate">
                        <span class="font-medium text-sm truncate">{{ Auth::user()->name }}</span>
                        <span class="text-xs text-white/70 truncate">{{ Auth::user()->email }}</span>
                    </div>
                    <svg class="ml-auto w-5 h-5 text-white/80 transition-transform duration-200 group-open:rotate-180" fill="none" stroke="currentColor" stroke-width="2"
                         viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"></path>
                ...
                <ul class="submenu border-t border-white/10 mt-2 pt-2">
                    <li>
                        <a wire:navigate href="{{ route('user.pass') }}"
                           class="block px-4 py-2 rounded hover:bg-white/10 text-sm flex items-center gap-3 transition-colors">
                            <svg class="w-5 h-5 text-white/85 shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                                 viewBox="0 0 24 24">
